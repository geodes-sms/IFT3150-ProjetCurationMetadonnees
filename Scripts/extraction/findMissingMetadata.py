...
def main(sr_df, do_web_scraping=False, run=999, sr_name=""):
    ...
    pd.DataFrame(erreurs, columns=['index', 'key', 'error']).to_excel(f"{MAIN_PATH}/Datasets/{sr_name}_erreurs_{str(run)}.xlsx" if sr_name else f"{MAIN_PATH}/Datasets/erreurs_{str(run)}.xlsx")
